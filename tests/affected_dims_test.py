...
def test_affected_dims_static(domain_3d: Domain):
    ...
    assert set(reshape.dims_affected((input_shape,))) == set((2, 3))

    input_shape = Shape((256, 512, 1, 32, 64))
    output_shape = Shape((256, 512, 32 * 64))
    reshape = top.ReshapeOp(OpId(0), domain_3d, {}, output_shape)
    assert set(reshape.dims_affected((input_shape,))) == set((2, 3, 4))

    input_shape = Shape((256, 512, 32, 64))
    output_shape = Shape((256 * 512, 32 * 64))
    reshape = top.ReshapeOp(OpId(0), domain_3d, {}, output_shape)
    assert set(reshape.dims_affected((input_shape,))) == set((0, 1, 2, 3))
    ...
    reshape = top.ReshapeOp(OpId(0), domain_3d, {}, output_shape)
    assert set(reshape.dims_affected((input_shape,))) == set((0,))

    input_shape = Shape((256, 512))
    output_shape = Shape((256, 4, 512 // 4))
    reshape = top.ReshapeOp(OpId(0), domain_3d, {}, output_shape)
    assert set(reshape.dims_affected((input_shape,))) == set((1,))
