...
This implementation contains naive mistakes such as what a developer could introduce
in an eager framework.
"""
...
class MHA(nn.Module):
    def __init__(
        ...
        self.key_cache = torch.zeros(
            (batch_size, num_heads, self.embed_size_per_head, seq_len), device=device
        )
    ...
    def forward(self, x: torch.Tensor, step: int) -> torch.Tensor:
        ...
        # Update KV cache
        # NOTE: bad tensor layout
        self.key_cache[:, :, :, step : step + 1] = k.squeeze(2).unsqueeze(3)
        self.value_cache[:, :, :, step : step + 1] = v.squeeze(2).unsqueeze(3)
        ...
        keys_to_use = keys_to_use.transpose(2, 3)
        values_to_use = values_to_use.transpose(2, 3)
...
class TorchBenchRunnerNaive(BenchRunner):
    def __init__(
        self,
        num_blocks: int,
        num_heads: int,
    ...
    def run(self):
        ...
        with torch.no_grad():
            ...
            for step in range(1, self.seq_len):
                token = output[:, :, step - 1 : step].squeeze(2).unsqueeze(1)  # Shape: (B, 1, E)
                assert token.shape == (
                    self.batch_size,
                    1,
                    self.embed_size,
                ), f"Token shape: {token.shape}"
                output[:, :, step : step + 1] = self.model(token, step).squeeze(1).unsqueeze(2)
