...
class ScheduleExecution(Transformation):
    ...
    def _run(self) -> Tuple[PDG, bool]:  # noqa: C901
        ...
        log.debug("Schedule constraint building time: %s", (end_time - start_time) / 1e9)
        ...
        try:
            ...
            end_time = time.perf_counter_ns()
            log.debug("Compute schedule time: %s", (end_time - start_time) / 1e9)
        ...
        if self.ctx.exec_cfg.enable_parallel_block_detection:
            ...
            def mark_set_nodes(node: islt.ScheduleNode) -> islt.ScheduleNode:
        ...
        if self.ctx.exec_cfg.enable_isolate_loop_conditions:
            ...
            def set_loop_types(node: islt.ScheduleNode) -> islt.ScheduleNode:
        ...
        # log.debug("Raise to tpo schedule time: %s", (end_time - start_time) / 1e9)
        ...
        if not self.quiet and self.ctx.exec_cfg.render_schedule:
            time_str = time.time()
            name = f"tpo_ast_{time_str}"
            tpo_schedule.render_to_dot(str(Path(self.ctx.exec_cfg.path) / f"{name}.dot"))
